...
for i in [56, 128, 256]:
    ...
    history = model_ResNet50.fit(dst_train, epochs = 100, validation_data = dst_val,
                              callbacks = [tf.keras.callbacks.EarlyStopping(patience=5),
                                           tf.keras.callbacks.ModelCheckpoint(filepath=f'ResNet50_MNIST_{i}.keras', save_best_only=True)])
